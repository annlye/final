...
public class OrderDao extends BaseDao {
    ...
    public List<Order> readod(int page) {
        ...
        try {
            String sql = "SELECT * FROM goodorder limit ?,?";
...
//            conn.close();
    ...
    public int findCount(){
        int  count=0;
        String sql="SELECT COUNT(id) as coun FROM goodorder";
        try {
            PreparedStatement pstmt = connection.prepareStatement(sql);
...
//            conn.close();
    ...
    public void deleteOrder(int num){
        try {
            String sql = "DELETE FROM goodorder WHERE num = ?";
            PreparedStatement pstmt=connection.prepareStatement(sql);
            pstmt.setInt(1,num);
            pstmt.execute();
        } catch (SQLException e) {
    ...
    public List<Order> selectLikeOrder(String title){
        ...
        Order od;
        try {
            String sql = "SELECT * FROM goodorder WHERE title like '%" + title + "%'";
